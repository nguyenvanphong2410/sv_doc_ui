...
function ModalUpdateDoc() {
  ...
  const handleChangeTypeUploadFile = (valueTypeUploadFile) => {
    if (valueTypeUploadFile === TYPE_SAVE.FILE) {
      dispatch(setInfoDocumentChecked({...infoDocumentChecked, type_save: TYPE_SAVE.FILE}));
    }
    if (valueTypeUploadFile === TYPE_SAVE.CHAPTERS) {
      dispatch(setInfoDocumentChecked({...infoDocumentChecked, type_save: TYPE_SAVE.CHAPTERS}));
    }
  };

  const removeContact = (index) => {
    let newChapter = [...infoDocumentChecked.chapters];
  ...
  const handleChapterChange = (index, field, value) => {
    ...
    const updatedChapters = _.cloneDeep(infoDocumentChecked.chapters);
  ...
  const addContact = () => {
    dispatch(
      setInfoDocumentChecked({
        ...infoDocumentChecked,
        chapters: [
          ...(infoDocumentChecked?.chapters?.length > 0 ? infoDocumentChecked.chapters : []),
          {
            name: '',
            number_order: 1,
            name_file_chapter: '',
        ...
        <Col sm={5} xs={24}>
          <div className={`input-wrap`}>
            ...
            <Radio.Group
              ...
              buttonStyle="solid"
              className="mt-[6px]"
              value={infoDocumentChecked.type_save}
              onChange={(e) => handleChangeTypeUploadFile(e.target.value)}
            />
          </div>
        </Col>
        <Col sm={19} xs={24}>
          {infoDocumentChecked?.type_save === TYPE_SAVE.FILE ? (
            <div className={`input-wrap`}>
              <div className="label-wrap">
            ...
            <>
              {infoDocumentChecked?.chapters?.length > 0 ? (
                <div className={`${styles.chaptersWrap} mt-[33px]`}>
                  {infoDocumentChecked?.chapters?.map((item, index) => (
              ...
              <div
                className={`flex justify-center ${
                  infoDocumentChecked?.chapter?.length > 0 ? 'mt-[16px]' : 'mt-[33px]'
                }`}
              >
                <Button
